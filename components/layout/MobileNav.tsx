...
const TABS = [
  ...
  {
    label: 'Create', href: '/create', special: true,
    icon: (_: boolean) => <span style={{ fontSize: 24, lineHeight: 1, color: 'white', fontWeight: 700 }}>+</span>,
  },
  {
          ...
          >
            {isCreate ? (
              <div style={{
                ...
                marginTop: -20, boxShadow: '0 4px 12px rgba(34,197,94,.4)',
              }}>
                {tab.icon(active)}
              </div>
            ) : (
            ...
            <span style={{ marginTop: isCreate ? 2 : 1 }}>{tab.label}</span>
          </Link>
        )
      })}
    </nav>
