...
export default function TopNav() {
  ...
  return (
    <nav className="topnav" style={{ display: 'flex', alignItems: 'center', padding: '0 24px', gap: 16 }}>
      {/* Hamburger — mobile only */}
      <button
        onClick={toggleDrawer}
        aria-label="Menu"
        style={{ display: 'none', width: 40, height: 40, alignItems: 'center', justifyContent: 'center', background: 'none', border: 'none', borderRadius: 8, flexShrink: 0 }}
      ...
      {/* Desktop nav links */}
      <div className="tn-nav-links" style={{ display: 'flex', gap: 4, marginLeft: 8 }}>
        {NAV.map(n => (
      ...
      <button
        onClick={openSearch}
        style={{
          flex: 1, maxWidth: 480, height: 44, borderRadius: 22,
      ...
      >
        ...
        <span style={{ flex: 1, fontSize: 13, color: 'var(--gray-400)' }}>Search NFTs, collections, and artists</span>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
        <button onClick={() => showToast('No new notifications')}
          style={{ width: 40, height: 40, borderRadius: '50%', background: 'var(--gray-50)', border: '1px solid var(--gray-200)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
        ...
        <button
          ...
          style={{ height: 40, padding: '0 18px', borderRadius: 20, background: 'var(--blue)', color: 'white', border: 'none', fontWeight: 600, fontSize: 14, transition: 'background .15s' }}
      ...
      <style>{`
        ...
        @media (max-width: 640px) {
          .tn-search { display: none !important; }
        }
      `}</style>
    </nav>
  )
